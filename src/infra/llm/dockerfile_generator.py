    ...
    def _build_detect_info(self, stack: Optional[str]) -> str:
        ...
        if stack == "nextjs":
            ...
            lines += [
                ...
                "**최적화 가이드 (필수):**",
                "1. **빌드 스테이지**: `FROM node:22-alpine AS builder`",
                "2. **필수 도구 설치**: yarn/pnpm/bun 사용 시 해당 도구 설치 (`RUN npm install -g pnpm` 등)",
                "3. **런타임 스테이지**: `FROM node:22-alpine`",
                "4. **⚠️ 패키지 매니저 재설치 (필수)**: builder에 설치된 pnpm/yarn/bun은 runner에 자동 복사되지 않음",
                "   - pnpm 사용 시: runner에 `RUN npm install -g pnpm && ...` (user 생성과 결합)",
                "   - yarn 사용 시: runner에 `RUN npm install -g yarn && ...`",
                "   - bun 사용 시: runner에 `RUN npm install -g bun && ...`",
                "   - `CMD [\"pnpm\", \"start\"]`를 쓰면서 runner에 pnpm이 없으면 pnpm: not found 에러 발생",
                "5. **production 의존성만**: `COPY --from=builder /app/node_modules ./node_modules`",
                "6. **소스 코드 복사**: `COPY --from=builder /app/src ./src` (또는 필요한 파일만)",
                "7. **레이어 결합**: 패키지 매니저 설치 + user 생성 + chown을 하나의 RUN에 결합",
